...
%Simulation stuffs:
time= 10000;           %Steps
doseSchedule = [1000,2000,3000,4000,5000,6000,7000,8000,9000];
...
drugDecay  = .0015;     %How much drug concentration decreases per day (1 is 100%).
dose = 1000;            %DrugDose
...
func = @(t,x)[
    r_tP * x(1) * log(K/(x(1) + x(2) + x(3))) - d_tP*x(1)* x(4) * testosteroneHelp- drugMurder*x(5);
    r_tM * x(2) * log(K/(x(1) + x(2) + x(3))) * x(4) * testosteroneHelp - d_tM*x(2)-drugMurder*x(5);
    r_tI * x(3) * log(K/(x(1) + x(2) + x(3))) - d_tI*x(3);
...
[t, data] = ode45(func,[0 doseSchedule(1)],[tP0, tM0, tI0, testosteroneInit, drugInit],options);
doseSchedule = [doseSchedule,time];
for i = 2:length(doseSchedule)      %Generates data
data(end) = data(end)+dose;
