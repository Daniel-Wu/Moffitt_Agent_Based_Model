...
for simulCount = 8:8%1:simulNum
    ...
    %Treatment Schedules
    AbiSchedule = [400];%[300 600 900 1200];
    testoSchedule = [800];%[500 700 900 1100 1300];
    doceSchedule = [500, 800];%[900];
    ...
    %Abi drug function
    param.Dose = 1000;         %Some sort of arbitrary param.drugDose.
    ...
    for i = 1:time                          %For the entire simulation
       ...
       for j=1:length(cellX)                %For every cell
          ...
          if luck<growthFactor                              %If you are lucky, you try to divide
    ...
    %SAVE DATA
    save([folderName '/' fileName num2str(simulCount)],'fieldData','testoFieldData','TMPop','TIPop','TPPop','testoTracker','drugTracker','doceTracker','PSATracker', 'AbiSchedule', 'testoSchedule','doceSchedule','param')
