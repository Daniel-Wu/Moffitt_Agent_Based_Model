...
%reproduction rates and drug resistance
%Daniel Wu
%6/19/16
...
emptyPhenotypes = zeros(phenotypeSize(1),phenotypeSize(2));
phenotypeCount = emptyPhenotypes;
phenotypeNum = numel(emptyPhenotypes);
...
%Drug function
dose = 1000;         %Some sort of arbitrary dose.
...
for i = 1:length(startField)
   for j = 1:length(startField) 
        startField(i,j) = ceil(rand()*phenotypeNum);
      ...
      cellType = field(cellX(j),cellY(j));              %And tell me what kind of cell it is. 
       ...
       %And it also might mutate
           ...
           if rand()<mutChance
               ...
               field(cellX(j),cellY(j)) = stdDevMut(cellType,phenotypeSize,phenotypeNum,deviation); %Mutate the cell
...
bar3(phenotypeCount);
title('Current Phenotypes');
xlabel('Drug Resistance');
ylabel('Base Growth Rate');
zlabel('Number of cells');
...
%histogram(phenotypeCount(zeroFree),'BinMethod','integers');